# MUST Service Record System v0.3.2
...
## v0.3.1
- 修正瀏覽器版 ExcelJS 載入範本時出現 `reading 'sheets'` 的相容性錯誤。
- 改用原始 `114-2服務紀錄.xlsm` 作為標準範本來源。
- 程式載入範本後再調整學生障別、備註與服務類型統計區塊。
- 服務紀錄空白列會複製原範本第 12 列的框線與格式。

## v0.3.2
- 移除瀏覽器載入 `.xlsm` 範本，修正 `reading 'sheets'` 錯誤。
- 改用 30 個細欄位建立固定 A4 版面，基本資料與服務紀錄各自獨立合併。
- 學生姓名、學生障別、科系／班級不再互相擠壓。
- 內容摘述欄固定使用 12 個細欄位，列高依實際全文精準調整。
- 不再需要上傳 `service-record-template.xlsm` 或 `.xlsx`。
